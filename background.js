...
Pomodoro.Timer = function Timer(pomodoro, options) {
  ...
  function tick() {
    timer.timeRemaining--;
    options.onTick(timer);
    if(timer.timeRemaining <= 0 || pomodoro.running == false) {

      last_known_type = options.type;

      clearInterval(tickInterval);
      pomodoro.onTimerEnd(timer);
      options.onEnd(timer);
      if(last_known_type == 'break' && pomodoro.opened == true) {
        mainPomodoro.start();

        // hard code
        chrome.browserAction.setIcon({
          path: ICONS.ACTION.CURRENT['work']
        });
      }
    }
  }
...
chrome.browserAction.onClicked.addListener(function (tab) {
  if(mainPomodoro.running) { 
    if(mainPomodoro.mostRecentMode == 'work') {
      ...
      mainPomodoro.opened = false;
      mainPomodoro.running = false;
    }
  } else {
      mainPomodoro.start();
  }
